...
SUPABASE_URL = os.environ.get("SUPABASE_URL", "https://btwbbjijrbyxbjlhtpqf.supabase.co")
SUPABASE_KEY = os.environ.get("SUPABASE_KEY", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ0d2JiamlqcmJ5eGJqbGh0cHFmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMzODIwNjEsImV4cCI6MjA4ODk1ODA2MX0.jkVvPfaEvhhWDg7zTdjKnll5gDqeyNBy3Eli4cxmbhQ")
...
def main(page: ft.Page):
    ...
    def login_view():
        ...
        return ft.View(route="/login", controls=[
            ft.Container(expand=True, padding=30, alignment=ft.alignment.center, content=ft.Column([
                ft.Icon(name="home_work", size=80, color="#1565C0"), ft.Text("Vibe ERP Mobile", size=24, weight="bold", color="#0D47A1"),
                ft.Divider(height=30, color="transparent"), u, p, ft.Container(height=10),
                ft.ElevatedButton(text="Login to System", on_click=do_login, width=float('inf'), height=50, bgcolor="#1565C0", color="white")
    ...
    def main_dashboard_view():
        def get_props_tab():
            ...
            if not items: lv.controls.append(ft.Container(content=ft.Text("ဒေတာမရှိသေးပါ။", color="grey"), padding=20, alignment=ft.alignment.center))
            for i in items:
    ...
    def add_prop_view():
        ...
        def save_data(e):
            ...
            if API.insert("apk_properties", {"title": title.value, "asking_price": float(price.value), "location": loc.value, "owner_name": owner.value, "is_deleted": 0}):
                show_snack("သိမ်းဆည်းပြီးပါပြီ"); navigate("/main")
            else: show_snack("Error", "red")

        return ft.View(route="/add_prop", bgcolor="white", controls=[ft.AppBar(title=ft.Text("အသစ်သွင်းရန်"), bgcolor="#1565C0", color="white"), ft.Container(padding=20, content=ft.Column([title, price, loc, owner, ft.ElevatedButton(text="Save Property", on_click=save_data, width=float('inf'), height=50, bgcolor="green", color="white")], spacing=15))])

    def route_change(e=None):
        page.views.clear()
        if not page.client_storage.get("auth") and page.route != "/login": page.route = "/login"
